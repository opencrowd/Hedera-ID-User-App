...
export default class FileViewer extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            canViewPhoto: this.props.navigation.getParam('canViewPhoto'),
            canViewName: this.props.navigation.getParam('canViewName'),
            canViewAge18: this.props.navigation.getParam('canViewAge18'),
            canViewAge21: this.props.navigation.getParam('canViewAge21'),
            canViewAddress: this.props.navigation.getParam('canViewAddress'),
            data: null,
            fontLoaded: false
        };
    }
    ...
    render() {
        ...
        if (this.state.data != null && this.state.data.content != null) {
            let profileData = JSON.parse(this.state.data.content);
             ...
             if (this.state.canViewPhoto) {
                photoData = 'data:image/jpg;base64,' + profileData.photo;
             };
             if (this.state.canViewName) {
                name = profileData.name;
             };
             if (this.state.canViewAge18) {
                is18 = profileData.is18;
             };
             if (this.state.canViewAge21) {
                is21 = profileData.is21;
             };
             if (this.state.canViewAddress) {
